...
import {QueryParams, deleteBattlestation} from '../../battlestations/apiQueries';
import {getAllBattlestations, IAdminBattlestationSummary} from './apiQueries';
import {authContext} from '../../useAuth';
import usePageTitle from '../../usePageTitle';
import AdminBattlestationListEntry from './AdminBattlestationListEntry';
import DeleteConfirmation from '../../components/DeleteConfirmation';
import {modalStyle} from '../../modalStyle';

function AdminBattlestations() {
  usePageTitle("Battlestation Admin | BattlestationDB");
  ...
  const [tempSearchTerm, setTempSearchTerm] = useState("");
	...
	const [queryParams, setQueryParams] = 
		useState<QueryParams>({pageNumber: 1, resultsPerPage: 12, sortBy: "created_date", direction: "asc", searchTerm: undefined});
  ...
  const [battlestationToDelete, setBattlestationToDelete] = useState<IAdminBattlestationSummary | null>(null);
  
  const {data: dataUserBattlestations, error: errorUserBattlestations, isLoading: isLoadingUserBattlestations} = 
    useQuery<{total: number, battlestations: IAdminBattlestationSummary[]}>(['userBattlestations'], async () => getAllBattlestations(queryParams, await getIdToken(authState.user!)));
  ...
  function handleDeleteClick(battlestation: IAdminBattlestationSummary) {
    if(authState.user) {
  ...
  return (
        ...
        <h2 className="text-4xl mb-4 font-semibold self-center">Battlestations</h2>
...
export default AdminBattlestations;